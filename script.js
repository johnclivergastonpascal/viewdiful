...
function loadVideo(index) {
  ...
  loadAd();       // ← AQUÍ SIEMPRE antes del video
  loadPart(current);
}
...
function resetAds() {
    ...
    // limpiar anuncios completamente
    container.innerHTML = "";

    // detener intervalo anterior
    if (window.adInterval) {
        clearInterval(window.adInterval);
        window.adInterval = null;
    }

    // volver a cargar anuncios frescos
    loadAd();
}
...
function loadAd() {
  const container = document.getElementById("ad-container");
  container.innerHTML = ""; // limpiar anuncios previos

  // crear los primeros anuncios
  createAds(1);

  // y cada 5 segundos crear 10 más (generación infinita)
...
function createAds(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    script2.src =
      "//www.highperformanceformat.com/9a8c9978fb1543656fe2727727a4e06f/invoke.js";
